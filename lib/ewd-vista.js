...
module.exports = {
  ...
  handlers: {
    ...
    checkDierr: function() {
      ...
      if (errNode.exists) {
        err.code = errNode.value;
        err.msg  = errNode.lastChild.firstChild.value;
      }
    ...
    listDic: function(query) {
      ...
      */
      ...
      for (let i = 0; i < query.fields.length; i++) {
        let fieldNode = new this.documentStore.DocumentNode('DD', [query.file.number, query.fields[i].number, '0']);
        let fieldName = fieldNode.value.split('^')[0];
    
        let field = {
          number: query.fields[i].number,
          name: fieldName,
      ...
      let fieldNums = query.fields.map(x => {return x.number;});
      let fieldsQry = '@;' +fieldNums.join(';');
      let fieldsMap = 'IEN^' + fieldNums.join('^');
